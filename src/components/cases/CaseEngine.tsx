...
export function CaseInvestigation({ creators, onComplete }: { creators: ReadonlyArray<{ id: string; name: string; creation: string; icon: string; detail: string; image?: string }>; onComplete: () => void }) {
  ...
  const mixed = ["leo", "zara", "mia", "ben"].map((id) => creators.find((item) => item.id === id)!).filter(Boolean);
  ...
  return <div className="rounded-[2rem] bg-white p-4 shadow-sm sm:p-7">
    <p className="text-center font-display text-sm font-bold uppercase tracking-[.18em] text-detective-orange-500">Case 1 · Investigation</p>
    <h2 className="mt-1 text-center font-display text-3xl font-bold text-detective-blue-900">WHO CREATED WHAT?</h2>
    <p className="mt-2 text-center text-detective-blue-700">Tap a creator, then tap the creation they made.</p>
    <Image src="/cases/world-1/who-created-what.png" alt="The four creators and their mixed-up creations at Idea Day" width={1672} height={941} className="mt-5 h-auto w-full rounded-2xl" />
    <div className="mt-6 grid gap-5 md:grid-cols-2">
      <div className="rounded-3xl bg-sky-50 p-4">
        <h3 className="mb-3 text-center font-display font-bold text-detective-blue-900">CREATORS</h3>
        <div className="grid gap-3">{creators.map((item) => <button key={item.id} type="button" disabled={found.includes(item.id)} onClick={() => { setCreator(item.id); setMessage(`Now find what ${item.name} created.`); }} aria-pressed={creator === item.id} className={"min-h-14 rounded-2xl border-2 px-5 py-3 text-left font-display text-lg font-bold focus-visible:outline focus-visible:outline-4 focus-visible:outline-detective-yellow-300 " + (found.includes(item.id) ? "border-green-500 bg-green-50 text-green-800" : creator === item.id ? "border-detective-orange-500 bg-detective-yellow-100 text-detective-blue-900" : "border-detective-blue-100 bg-white text-detective-blue-900")}>{item.name}{found.includes(item.id) && " ✓"}</button>)}</div>
      </div>
      <div className="rounded-3xl bg-detective-yellow-50 p-4">
        <h3 className="mb-3 text-center font-display font-bold text-detective-blue-900">CREATIONS — MIXED UP</h3>
        <div className="grid gap-3">{mixed.map((item) => <button key={item.id} type="button" disabled={found.includes(item.id)} onClick={() => chooseCreation(item.id)} className={"min-h-14 rounded-2xl border-2 px-5 py-3 text-left font-display text-lg font-bold focus-visible:outline focus-visible:outline-4 focus-visible:outline-detective-yellow-300 " + (found.includes(item.id) ? "border-green-500 bg-green-50 text-green-800" : "border-detective-blue-100 bg-white text-detective-blue-900")}>{item.creation}{found.includes(item.id) && " ✓"}</button>)}</div>
      </div>
    </div>
    ...
    {found.length === creators.length && <div className="mt-4 text-center"><p className="font-display text-xl font-bold text-detective-orange-600">🎉 ALL CLUES FOUND!</p><button type="button" onClick={onComplete} className="mt-4 min-h-12 rounded-full bg-detective-orange-500 px-7 py-3 font-display font-bold text-white">CONTINUE THE CASE <ArrowRight className="inline h-4 w-4" /></button></div>}
  </div>;
}
...
export function CaseQuestion({ question, index, total, onComplete }: { question: { prompt: string; choices: readonly string[]; correct: number; feedback: string }; index: number; total: number; onComplete: () => void }) {
  ...
  const advance = () => { setFeedback(""); setCorrect(false); onComplete(); };
