...
export function CaseHeader({ nickname, detective }: { nickname: string; detective: DetectiveProfile | null }) {
  return <header className="grid items-center gap-4 rounded-[2rem] bg-detective-blue-900 px-5 py-5 text-white sm:grid-cols-[auto_1fr_auto]">
    ...
    <div className="text-center sm:text-left"><p className="font-display text-xs font-semibold uppercase tracking-[.18em] text-detective-yellow-300">Creator Park · Case 1</p><h1 className="mt-1 font-display text-3xl font-bold">QUESTY&apos;S FIRST MYSTERY</h1><p className="mt-2 text-white/85">{nickname ? `Detective ${nickname}, we have a mystery!` : "Detective, we have a mystery!"}</p></div>
...
export function CaseIntro({ nickname, detective, onStart }: { nickname: string; detective: DetectiveProfile | null; onStart: () => void }) {
  return <div className="space-y-6 text-center"><CaseHeader nickname={nickname} detective={detective} /><div className="rounded-3xl bg-white p-6 shadow-sm"><p className="font-display text-sm font-bold uppercase tracking-[.18em] text-detective-orange-500">The mystery</p><h2 className="mt-2 font-display text-3xl font-bold text-detective-blue-900">The Creator Cards are mixed up!</h2><p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed text-detective-blue-800">Four young creators are getting ready for Idea Day. We know what was created, but we need to find out who created each one.</p><p className="mt-4 font-display text-xl text-detective-blue-900">Can you help Questy solve the mystery?</p><button type="button" onClick={onStart} className="mt-6 inline-flex min-h-12 items-center gap-2 rounded-full bg-detective-orange-500 px-7 py-3 font-display text-lg font-bold text-white shadow-lg focus-visible:outline focus-visible:outline-4 focus-visible:outline-detective-yellow-300">START INVESTIGATION <span aria-hidden>🔎</span></button></div></div>;
}
