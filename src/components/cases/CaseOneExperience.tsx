...
import { CaseColourReward, CaseFile, CaseHeader, CaseIntro, CaseInvestigation, CaseJigsawReward, CaseQuestion, CaseShell, CaseVideo, type CaseStep } from "@/components/cases/CaseEngine";
...
export function CaseOneExperience({ isCompleted, onComplete }: { isCompleted: boolean; onComplete: (correct: number, total: number) => void }) {
  ...
  if (step === "story") return <CaseShell step={step}><CaseVideo source={caseOne.video} slides={caseOne.story} onReady={() => setStep("investigate")} /></CaseShell>;
  ...
  if (step === "discovery") return <CaseShell step={step}><div className="rounded-3xl bg-white p-6 text-center shadow-sm"><CaseHeader nickname={nickname} detective={detective} /><p className="mt-6 font-display text-sm font-bold uppercase tracking-[.18em] text-detective-orange-500">Detective discovery</p><h2 className="mt-2 font-display text-3xl font-bold text-detective-blue-900">People create things every day.</h2><div className="mt-5 grid gap-3 sm:grid-cols-3"><p className="rounded-2xl bg-sky-50 p-4 font-display font-bold text-detective-blue-900">🎨 Drawings</p><p className="rounded-2xl bg-detective-yellow-50 p-4 font-display font-bold text-detective-blue-900">📖 Stories</p><p className="rounded-2xl bg-sky-50 p-4 font-display font-bold text-detective-blue-900">💡 Inventions</p></div><p className="mx-auto mt-5 max-w-2xl text-lg leading-relaxed text-detective-blue-800">People use their ideas, imagination and skills to make creations. Names, signs and designs can be creations too.</p><div className="mx-auto mt-6 max-w-xl rounded-3xl border-2 border-detective-orange-300 bg-detective-yellow-50 p-5"><p className="font-display text-sm font-bold uppercase tracking-[.18em] text-detective-orange-600">New detective word</p><h3 className="mt-1 font-display text-3xl font-bold text-detective-blue-900">IP</h3><p className="font-display text-xl font-bold text-detective-blue-800">Intellectual Property</p><p className="mt-3 text-detective-blue-800">IP is short for Intellectual Property. It helps us talk about creations of the mind and the different ways some creations may be protected.</p></div><p className="mt-5 text-detective-blue-700">Don&apos;t worry — you don&apos;t need to remember everything yet! We&apos;ll discover more as we solve cases around Idea City.</p><button type="button" onClick={() => setStep("clues")} className="mt-6 inline-flex items-center gap-2 rounded-full bg-detective-orange-500 px-7 py-3 font-display font-bold text-white">CHECK THE CLUES 🔎 <ArrowRight className="h-4 w-4" /></button></div></CaseShell>;
...
function CaseSolved({ nickname, detective, onReward, onFile }: { nickname: string; detective: DetectiveProfile | null; onReward: () => void; onFile: () => void }) {
  const reduced = useReducedMotion();
  return <motion.div initial={reduced ? false : { opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} className="overflow-hidden rounded-3xl bg-gradient-to-br from-detective-yellow-100 via-white to-sky-100 p-6 text-center shadow-sm"><div aria-hidden className="text-3xl">{reduced ? "✨" : "✨ 🎉 ⭐ 🎉 ✨"}</div><div className="mx-auto mt-2 grid max-w-md items-center gap-3 sm:grid-cols-2"><Image src={questyArt.celebrating} alt="Questy celebrating" className="mx-auto h-36 w-auto object-contain" />{detective && <DetectiveAvatar avatar={detective.avatar} pose="celebrate" />}</div><h2 className="mt-3 font-display text-4xl font-bold text-detective-blue-900">🎉 CASE SOLVED!</h2><p className="mt-2 font-display text-xl font-bold text-detective-orange-600">YOU SOLVED QUESTY&apos;S FIRST MYSTERY!</p><p className="mx-auto mt-4 max-w-xl text-lg text-detective-blue-800">Fantastic detective work{nickname ? `, Detective ${nickname}` : ""}! You found the creators, followed the clues, and solved the mystery.</p><div className="mx-auto mt-6 max-w-xl rounded-3xl border-2 border-detective-orange-300 bg-white p-5"><p className="font-display text-sm font-bold uppercase tracking-[.18em] text-detective-orange-500">First rule of Idea City</p><p className="mt-2 font-display text-2xl font-bold text-detective-blue-900">⭐ CREATORS MATTER.<br />THEIR CREATIONS MATTER TOO.</p></div><div className="mt-6 flex flex-wrap justify-center gap-3"><button type="button" onClick={onReward} className="rounded-full bg-detective-orange-500 px-6 py-3 font-display font-bold text-white">🎁 DETECTIVE REWARD</button><button type="button" onClick={onFile} className="inline-flex items-center gap-2 rounded-full border-2 border-detective-blue-200 bg-white px-6 py-3 font-display font-bold text-detective-blue-800"><FileText className="h-4 w-4" />VIEW CASE FILE</button></div></motion.div>;
}
