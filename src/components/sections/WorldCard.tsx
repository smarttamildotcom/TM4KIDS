...
/** Compact journey card. Each World opens on its own dedicated route. */
export function WorldCard({ world, status, isActive, onRequestUnlock, caseTitle }: WorldCardProps) {
  ...
  return (
    <article id={`world-${world.id}`} className={`relative w-full scroll-mt-24 rounded-[22px] bg-gradient-to-br p-[3px] shadow-lg transition duration-300 ${theme.gradient} ${isLocked ? "opacity-60 grayscale" : `hover:-translate-y-1 hover:shadow-2xl ${theme.glow}`} ${isActive ? "ring-4 ring-detective-yellow-300" : ""}`}>
      {isLocked && <LockedWorldOverlay worldName={world.name} onRequestUnlock={onRequestUnlock} />}
      <span aria-hidden="true" className="pointer-events-none absolute -right-2 -top-3 text-lg opacity-70">{isCompleted ? "🌟" : world.id === 15 ? "💎" : "🐾"}</span>

      <div className={`flex flex-col rounded-[19px] border bg-white p-3 ${theme.border}`}>
        <div className="flex items-start gap-2">
          <div className="relative shrink-0">
            <ProgressRing percent={percent} className={theme.ring} />
            <span className="absolute inset-0 grid place-items-center font-display text-sm font-bold text-detective-blue-900">{world.id}</span>
          </div>
          <div className="min-w-0 grow">
            <div className="flex flex-wrap items-center gap-1">
              <span aria-hidden="true" className="text-lg">{world.icon}</span>
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${difficultyChip[world.difficulty]}`}>{world.difficulty}</span>
            </div>
            {world.id === 1 && caseTitle && <p className="mt-0.5 font-display text-[9px] font-bold uppercase tracking-[.1em] text-detective-orange-500">Case 1 · Creator Park</p>}
            <h3 className="mt-0.5 line-clamp-2 font-display text-[15px] font-bold leading-tight text-detective-blue-900">{title}</h3>
          </div>
          <Image src={art.src} alt={art.alt} sizes="56px" className="h-14 w-14 shrink-0 object-contain drop-shadow-md" />
        </div>

        <p className="mt-1.5 line-clamp-2 text-[11px] leading-snug text-detective-blue-700/80">{world.id === 1 && caseTitle ? "Help Questy solve the first Creator Park mystery." : world.description}</p>
        ...
        <div className="mt-2">
          {isLocked ? (
            <button type="button" onClick={onRequestUnlock} className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-detective-blue-200 px-3 py-2 font-display text-[11px] font-bold text-detective-blue-800"><Lock className="h-3.5 w-3.5" /> Unlock World</button>
          ) : (
            <Link href={`/worlds/${world.id}`} className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-detective-orange-500 px-3 py-2 font-display text-[11px] font-bold text-white shadow-md transition-colors hover:bg-detective-orange-600"><Play className="h-3.5 w-3.5" /> {isCompleted ? "Replay World" : "Start World"}</Link>
          )}
        </div>
      </div>
    </article>
