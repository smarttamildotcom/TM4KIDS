...
/** Compact square journey card. Each World opens on its own dedicated route. */
export function WorldCard({ world, status, isActive, onRequestUnlock, caseTitle }: WorldCardProps) {
  ...
  return (
    <article id={`world-${world.id}`} className={`relative aspect-square w-full scroll-mt-24 rounded-[24px] bg-gradient-to-br p-[3px] shadow-lg transition duration-300 ${theme.gradient} ${isLocked ? "opacity-60 grayscale" : `hover:-translate-y-1 hover:shadow-2xl ${theme.glow}`} ${isActive ? "ring-4 ring-detective-yellow-300" : ""}`}>
